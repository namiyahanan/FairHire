...
const ApplicationStatus = () => {
  ...
  // Assessment submission handler (attends round with HR timing slot selection)
  const handleCompleteAssessment = () => {
    ...
    const chosenSlotText = typeof chosenSlotObj === 'string' ? chosenSlotObj : chosenSlotObj?.text || 'Confirmed Slot';

    setTimeout(() => {
      const currentCandId = currentApp?.candidateId || activeCandidateId;
      const updated = candidateAttendRound(currentCandId, roundIdx, chosenSlotText);
      setCandidateHiring(updated);
      setAssessmentSubmitting(false);
      setShowAssessmentModal(false);
      const roundName = updated.rounds?.[roundIdx]?.name || `Round ${roundIdx + 1}`;
      const score = updated.rounds?.[roundIdx]?.result?.score || 92;
      setAssessmentCompletedSuccess(
        `✓ Assessment for "${roundName}" completed with score ${score}%! Evaluator results automatically delivered to HR (Slot: ${chosenSlotText}).`
      );
      setTimeout(() => {
  ...
  return (
    <DashboardLayout>
              ...
              <div className="flex items-start sm:items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-300 mt-0.5 max-w-2xl">
                    {activeRound.description} — The HR team has approved your profile for this round. Please review the completion timeframe and select your preferred assessment slot below.
                  </p>
                </div>
              </div>
            ...
            <div className="pt-5 relative z-10">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-3 border-t border-navy-800">
                ...
                >
                  ...
                  <span>Confirm Slot & Attend Assessment Solution</span>
                          ...
                          <div>
                            {isCompleted && (
                              <span className="text-xs font-bold text-emerald-800 bg-emerald-100 border border-emerald-300 px-3 py-1 rounded-full flex items-center gap-1.5">
                                ...
                                <span>Approved / Passed ({round.result?.score}%)</span>
                              </span>
                            )}
                            {isWaitingCandidate && (
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <div>
                                  <h6 className="text-xs sm:text-sm font-black text-navy-900">
                                    Interview Schedule & Timing Slots Posted by HR
                                  </h6>
                                  <p className="text-[11px] text-slate-500">
                                    The HR team has approved you for this round and posted available timing slots.
                                  </p>
                                </div>
                              </div>
                          ...
                          <div className="mt-3 p-3.5 rounded-2xl bg-emerald-50/50 border border-emerald-200 text-xs text-emerald-950 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                            <div>
                              <div className="flex items-center gap-2 font-bold text-emerald-900">
                                <CheckCircle2 className="w-4 h-4 text-emerald-600" />
                                <span>Evaluation Score: {round.result.score}% • Evaluated by {round.result.evaluatedBy}</span>
                              </div>
                              <p className="text-[11px] text-emerald-800 mt-0.5">
                                <em>"{round.result.feedback}"</em>
                              </p>
                            </div>

                            {round.selectedSlot && (
                              <span className="text-[11px] text-slate-500 font-mono bg-white px-2.5 py-1 rounded-lg border border-emerald-200 shrink-0">
                                Scheduled: {round.selectedSlot}
                              </span>
                            )}
                          </div>
                        )}
      ...
      {/* ================= INTERACTIVE ROUND ASSESSMENT MODAL ================= */}
      {showAssessmentModal && activeRound && (
        <div className="fixed inset-0 z-50 bg-navy-950/75 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl max-w-2xl w-full p-6 sm:p-8 relative overflow-hidden">
            ...
            <div className="flex items-start justify-between gap-4 pb-5 border-b border-slate-100">
              <div className="flex items-center gap-3.5">
                <div className="w-12 h-12 rounded-2xl bg-teal-600 text-white flex items-center justify-center font-black shadow-sm shrink-0">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-bold text-teal-700 bg-teal-50 border border-teal-200 px-2.5 py-0.5 rounded-full uppercase">
                      Round {currentRoundIdx + 1} Assessment
                    </span>
                    <span className="text-xs text-slate-400">Candidate ID: CAND-8492</span>
                  </div>
                  <h3 className="text-lg sm:text-xl font-extrabold text-navy-900 mt-0.5">
                    {activeRound.name}
                  </h3>
                </div>
              </div>
            ...
            {/* Modal Content */}
            <div className="space-y-4 py-5 text-xs text-slate-600">
              {/* Scheduled Timing Slot & Deadline Banner */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                <div className="p-3 rounded-xl bg-teal-50 border border-teal-200">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-teal-800 block mb-0.5">
                    Selected Assessment Slot
                  </span>
                  <p className="font-mono text-teal-900 font-bold text-xs flex items-center gap-1.5">
                    <Calendar className="w-3.5 h-3.5 text-teal-600" />
                    <span>
                      {typeof (activeRound.invitation?.timingSlots?.[selectedSlotIndex] || (activeRound.invitation?.timingSlots?.[0])) === 'string'
                        ? (activeRound.invitation?.timingSlots?.[selectedSlotIndex] || activeRound.invitation?.timingSlots?.[0])
                        : (activeRound.invitation?.timingSlots?.[selectedSlotIndex]?.text || activeRound.invitation?.timingSlots?.[0]?.text || 'Confirmed Slot')}
                    </span>
                  </p>
                </div>
                ...
                <div className="p-3 rounded-xl bg-amber-50 border border-amber-200">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-amber-800 block mb-0.5">
                    Submission Timeframe / Deadline
                  </span>
                  <p className="text-amber-900 font-bold text-xs flex items-center gap-1.5">
                    <Clock className="w-3.5 h-3.5 text-amber-600" />
                    <span>{activeRound.invitation?.deadlineDate || 'Within 3 days'} ({activeRound.invitation?.deadlineDays || 3} days)</span>
                  </p>
                </div>
              </div>

              <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200">
                <h5 className="font-extrabold text-navy-900 text-xs mb-1 flex items-center gap-1.5">
                  <Info className="w-4 h-4 text-teal-600" />
                  Round Instructions & Objectives:
                </h5>
              ...
              {/* Simulated Live Assessment Challenge */}
              <div className="p-4 rounded-2xl bg-navy-950 text-slate-200 font-mono text-xs border border-navy-800">
                <div className="flex items-center justify-between pb-2 border-b border-navy-800 text-[11px] text-slate-400 mb-3">
                  <span className="text-teal-400 font-bold">Assessment Console • Live Environment</span>
                  <span>Time Allocated: 45 Mins</span>
                </div>
                <p className="text-slate-300 mb-2">
                  <span className="text-teal-400">// Challenge:</span> Complete practical engineering assessment for {currentApp.jobTitle}.
                </p>
                <div className="p-3 rounded-xl bg-navy-900 border border-navy-800 text-[11px] text-emerald-300">
                  <p>✓ Problem statement loaded: Modular Architecture & System Verification</p>
                  <p>✓ All 12 automated unit test fixtures passed (100% test coverage)</p>
                  <p>✓ Memory profiling: Zero leaks detected, optimal O(1) runtime</p>
                  <p className="text-teal-300 mt-1">✓ Solution validated and ready for HR evaluation review.</p>
                </div>
              </div>

              <div className="p-3.5 rounded-xl bg-teal-50 border border-teal-200 text-teal-800 text-xs">
                <p className="font-bold flex items-center gap-1.5 mb-0.5">
                  <Sparkles className="w-3.5 h-3.5 text-teal-600" />
                  Automatic Result Synchronization
                </p>
                <p className="text-[11px] text-teal-700">
                  Upon clicking submit, your test result, performance score, and evaluator remarks will automatically be transmitted to the HR recruiter dashboard in real-time.
                </p>
              </div>
            </div>
            ...
            <div className="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
              ...
              <Button
                ...
                className="shadow-md"
              >
                Submit Assessment Solution & Send to HR
              </Button>
            </div>
          </div>
        </div>
